...
class TDEGrid:
    def __init__(
        ...
        sigma_coeff : float
            The sigma coefficient to be used in computing the scale radius, defined as a = sigma_coeff * G * M_s / sigma^2. If not provided, it is set to 1/18 by default.
    ...
    def _get_a(self, sigma_coeff, M_s, sigma):
    ...
    def _get_single_TDE_rates(self, M_s_scaler):
        ...
        _as = (
            np.full_like(self.M_bhs, self.a)
            if self.a is not None
            else self._get_a(self.sigma_coeff, self.M_bhs * M_s_scaler, self.sigma)
        )
...
def KH13_logMs_from_logMbh(log_Mbh):
    return 11 + (np.log10(1 / 0.49) + (log_Mbh - 9)) / 1.17
...
class AllSkyTDERate:
    ...
    def _get_all_sky_tde_rate(self):
        ...
        self._log_tde_rate_Mbh_bins = np.log10(
            np.nansum(
                10**_log_tde_rate
